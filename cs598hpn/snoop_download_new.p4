...
// typedef bit<OPTION_SIZE> raw_tcp_option_t;
...
parser MyParser(packet_in packet,
    ...
    state parse_payload {
        // packet.extract(hdr.payload, (bit<32>)((hdr.ipv4.totalLen - 40) * 8));
...
control MyIngress(inout headers hdr,
    ...
    register<raw_tcp_option_t>(BLOOM_FILTER_ENTRIES) cache_tcp_option;
    register<bit<PAYLOAD_SIZE>>(BLOOM_FILTER_ENTRIES) cache_payload;
    // register<varbit<PAYLOAD_SIZE>>(BLOOM_FILTER_ENTRIES) cache_payload;
    ...
    // raw_tcp_option_t raw_tcp_option;
    bit<1> dupAck = 0;
    bit<1> data_packet = 0;
    ...
    action generate_tcp_hdr() {
        ...
        // cache_tcp_option.read(raw_tcp_option, reg_pos);
        // hdr.tcp_opt.padding             = raw_tcp_option[OPTION_PADDING+63:64];
        // hdr.tcp_opt.sack_left_edge      = raw_tcp_option[63:32];
        // hdr.tcp_opt.sack_right_edge     = raw_tcp_option[31:0];
    }
    ...
    action record_tcp_hdr() {
        ...
        // raw_tcp_option[OPTION_PADDING+63:64]    = hdr.tcp_opt.padding;
        // raw_tcp_option[63:32]                   = hdr.tcp_opt.sack_left_edge;
        // raw_tcp_option[31:0]                    = hdr.tcp_opt.sack_right_edge;
        // cache_tcp_option.write(reg_pos, raw_tcp_option);
    }
    ...
    table debug_table {
        key = {
            ...
            // hdr.tcp_opt.isValid(): exact;
            hdr.payload.isValid(): exact;
        }
        actions = { NoAction; }
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            if (hdr.ipv4.dstAddr == 0x0a000202 && hdr.tcp.isValid()) { // Only proccess packet to receiver (hardcoded) with IP 10.0.2.2
                ...
                if (hdr.ipv4.totalLen == 0x05dc) { // Move the check here instead of parsing
                    data_packet = 1;
            ...
            // Find duplicate ACK and drop it, and retransmit lost segment 
                ...
                if (hdr.tcp.seqNo == prev_seq && hdr.tcp.ackNo == prev_ack) {
                    ...
                    // hdr.tcp_opt.setInvalid(); // Has tried to invalidate tcp option

                    cache_payload.read(hdr.payload.data, reg_pos);
                    // hdr.payload.setValid(); // It leads to the error that it exceeds the packet size limit
            ...
            if (dupAck == 1) {
                // send_back();
                ipv4_forward(hdr.ethernet.srcAddr, standard_metadata.ingress_port); // Same functionality as send_back
            } else {
                ipv4_lpm.apply();
            }
        }
